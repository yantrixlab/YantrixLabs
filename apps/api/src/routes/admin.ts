...
router.get(
  ...
  async (_req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const [totalUsers, totalBusinesses, totalInvoices, totalRevenue] =
        await Promise.all([
          prisma.user.count(),
    ...
    try {
      const review = await prisma.review.update({
        ...
        data: {
          ...
          repliedAt: req.body.reply ? new Date() : undefined,
        },
      });
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (mod.isCore) {
        res
          .status(400)
          .json({ success: false, error: "Cannot delete core modules" });
        return;
      }
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const { endDate: planEndDate, amount: planAmount } =
        getPlanBillingDetails(plan);
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!startDate && !endDate) {
        res
          .status(400)
          .json({ success: false, error: "startDate or endDate required" });
        return;
      }
      ...
      if (existing.isSystem) {
        if (data.slug && data.slug !== existing.slug) {
          res
            ...
            .json({
              success: false,
              error: "Cannot change slug of a system tool",
            });
          return;
        }
        if (data.status && data.status !== "PUBLISHED") {
          res
            ...
            .json({
              success: false,
              error: "System tools must remain published",
            });
          return;
        }
        if (data.visibility && data.visibility !== "PUBLIC") {
          res
            .status(403)
            .json({ success: false, error: "System tools must remain public" });
          return;
        }
      }
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (existing.isSystem) {
        res
          .status(403)
          .json({ success: false, error: "System tools cannot be deleted" });
        return;
      }
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const original = await prisma.tool.findUnique({
        where: { id: req.params.id },
      });
      if (!original) {
        res.status(404).json({ success: false, error: "Tool not found" });
      ...
      const { id, createdAt, updatedAt, viewCount, clickCount, ...rest } =
        original;
...
router.get(
  ...
  async (_req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      res.json({
        success: true,
        data: config ?? { key: ABOUT_STATS_KEY, ...ABOUT_STATS_DEFAULTS },
      });
    } catch (error) {
      next(error);
    }
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!name || !role || !bio) {
        res
          .status(400)
          .json({ success: false, error: "name, role, and bio are required" });
        return;
      }
...
const CONTACT_DETAILS_DEFAULTS = {
  ...
  hoursNote:
    "Extended support hours during GST filing deadlines (20th – 22nd of each month).",
};
