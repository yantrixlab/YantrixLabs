  ...
  return (
    <section className="relative overflow-hidden border-b border-cyan-500/10 bg-[#0b1020]">
      <div
        ...
        style={{
          background:
            'radial-gradient(circle at 14% 12%, rgba(59,130,246,0.24), transparent 34%), radial-gradient(circle at 86% 16%, rgba(168,85,247,0.22), transparent 42%), linear-gradient(180deg, #0b1020 0%, #101a33 100%)',
        }}
      />
      ...
      <div className="container-wide relative py-20 lg:py-24">
        <div className="grid items-center gap-12 lg:grid-cols-2">
          ...
          <motion.div
            ...
            className="rounded-3xl border border-cyan-400/20 bg-slate-950/70 p-4 shadow-2xl shadow-blue-950/40 backdrop-blur"
          >
            <div className="rounded-2xl border border-slate-700/70 bg-slate-900/90 p-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className="rounded-full bg-emerald-400/20 px-2.5 py-1 text-xs font-semibold text-emerald-300">Connected</span>
              </div>

              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-xl border border-violet-400/30 bg-slate-900 p-3 text-center">
                  <QrCode className="mx-auto h-5 w-5 text-violet-300" />
                  <p className="mt-2 text-xs font-semibold text-slate-200">Step 1</p>
                  <p className="text-xs text-slate-400">Scan Pair QR</p>
                </div>
                <div className="rounded-xl border border-cyan-400/30 bg-slate-900 p-3 text-center">
                  <ScanLine className="mx-auto h-5 w-5 text-cyan-300" />
                  <p className="mt-2 text-xs font-semibold text-slate-200">Step 2</p>
                  <p className="text-xs text-slate-400">Scan Product</p>
                </div>
                <div className="rounded-xl border border-indigo-400/30 bg-slate-900 p-3 text-center">
                  <CheckCircle2 className="mx-auto h-5 w-5 text-indigo-300" />
              ...
              <div className="mt-4 rounded-xl border border-slate-700 bg-slate-950/80 p-3">
                <p className="text-xs font-semibold text-slate-400">Recent Scan</p>
                <div className="mt-2 flex items-center justify-between text-sm">
                  <span className="font-medium text-slate-200">Premium Packaging Box</span>
                  <span className="font-semibold text-white">₹1,200</span>
                </div>
                <div className="mt-1 flex items-center justify-between text-xs text-slate-400">
                  <span>GST 18% • Qty 1</span>
                  <span className="font-semibold text-emerald-300">Added to invoice</span>
                </div>
              </div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div className="rounded-xl border border-slate-700 bg-slate-950/80 p-3">
                  ...
                  <p className="mt-1 text-sm font-semibold text-slate-100">-1 Unit Auto Updated</p>
                </div>
                <div className="rounded-xl border border-slate-700 bg-slate-950/80 p-3">
                  <p className="text-xs font-semibold text-slate-500">Export</p>
                  <p className="mt-1 text-sm font-semibold text-slate-100">PDF + Excel Ready</p>
                </div>
              </div>
            </div>
          </motion.div>
